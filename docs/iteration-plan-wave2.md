...
Iter 10 ──→ Iter 11 ──→ Iter 12
  (KAG       (LightRAG    (Backend
   cleanup)   full stack)   hardening)

Iter 13 ──→ Iter 14
  (Frontend    (Design
   resilience)  system)

Iter 15 (CI/CD) — independent, can run in parallel with 13-14
Iter 16 (Polish) — after all above
```

**Critical path:** 10 → 11 → 12 (must be sequential)
**Parallel track:** 13 + 14 can start after 10, in parallel with 11
...
| **12** | Backend Security & Reliability | HIGH | 10 | 11 | M |
| **13** | Frontend Resilience & Auth | HIGH | 9 | 10 | M |
| **14** | Design System & Accessibility | MEDIUM | 10 | 13 | M |
| **15** | CI/CD & Infrastructure | MEDIUM | 5 | — | S |
| **16** | Polish & Technical Debt | LOW | 7 | 12, 14 | S |
| | **Total** | | **60** | | |
...
**Blocks:** Iter 11 (LightRAG uses tables freed by KAG cleanup)
...
| 10.3 | Delete `/kag/*` API routes | backlog | ~118 LOC in `index.js`. Currently returns 410 in LIGHTRAG_ONLY mode. |
| 10.4 | Clear scheduler of KAG jobs | backlog | Remove KAG job types from CASCADE_CHAINS and seed data. |
| 10.5 | DROP unused KAG DB tables | backlog | `kag_nodes`, `kag_edges`, `kag_provenance_refs`, `kag_signal_state`, `kag_signals`, `kag_signal_history`, `kag_scores`, `kag_score_history`, `kag_recommendations`, `kag_templates`. |
...
**Blocks:** Iter 12 (backend hardening for new LightRAG services)
...
| 11.6 | Proxy LightRAG query endpoints | backlog | Rewrite `POST /lightrag/query` to call HKUDS LightRAG `/query` with mode (naive/local/global/hybrid). Proxy status and health. |
...
**Blocked by:** Iter 11 (new services need hardening)
...
**Exit criteria:** All 10 fixes applied. No raw error messages in API responses (verified by grep). Scheduler safe for 2+ concurrent workers.
...
| 13.6 | Introduce unified data fetching layer | FE-06 | Add `@tanstack/react-query`. Wrap all `apiFetch` calls in `useQuery`/`useMutation`. Get: automatic caching, deduplication, stale-while-revalidate, retry, loading/error states. |
...
**Exit criteria:** No blank screens on errors. Unauthenticated users never see protected UI. All pages have titles in browser tab. react-query devtools show cache hits.
...
**Blocked by:** nothing — can run in parallel with Iter 13-14.
**Blocks:** nothing
...
**Exit criteria:** CI blocks on `npm audit` high. Deploy creates backup before apply. Rollback tested at least once.
...
**Priority:** LOW
**Goal:** Remaining medium/low issues, documentation, cleanup.
**Blocked by:** Iter 12, 14
**Blocks:** nothing
...
| 16.5 | Return 404 for KAG routes in LIGHTRAG_ONLY | BE-12 | Change 410 → 404 response. Don't confirm feature existence. |
| 16.6 | Add retry/backoff on frontend API calls | FE-11 | Configure react-query defaults: `retry: 3`, `retryDelay: exponentialBackoff`, only for 5xx/network errors. |
| 16.7 | Remove PageLoadingSkeleton infinite loop | DS-05 | Replace `loop: true` with single iteration or 2-cycle fade. Align with MOTION_GUIDELINES. |
...
Week 1     ┃ Iter 10 — KAG Cleanup + DB Hygiene
           ┃
Week 2-3   ┃ Iter 11 — Full LightRAG Integration
           ┃    ├── 11.1-11.3  Schema + ingestion pipeline
           ┃    ├── 11.4-11.6  HKUDS LightRAG server + proxy
           ┃    └── 11.7-11.10 ACL, citations, MCP, tests
           ┃
Week 3-4   ┃ Iter 12 — Backend Hardening     ║ Iter 15 — CI/CD (parallel)
           ┃                                  ║
Week 4-5   ┃ Iter 13 — Frontend Resilience    ║
           ┃                                  ║
Week 5-6   ┃ Iter 14 — Design & Accessibility ║
           ┃                                  ║
Week 6     ┃ Iter 16 — Polish
           ┃
           ┗━━━ Target: 97%+ maturity ━━━━━━━━┛
```
...
| Issue ID | Description | Iter |
|----------|------------|------|
| DB-01 | Duplicate migration numbering (0017) | 10.7 |
| DB-02 | Missing ON DELETE CASCADE | 16.1 |
| DB-03 | Legacy KAG tables | 10.5 |
| DB-04 | Partial index pending embeddings | 10.8 |
| DB-05 | mv_portfolio_dashboard refresh | 15.5 |
| DB-06 | audit_events_partitioned unused | 10.9 |
| BE-01 | Embedding state not transactional | 12.1 |
| BE-02 | Login rate limit bypass | 12.2 |
| BE-03 | Error messages leak internals | 12.3 |
| BE-04 | No idempotency keys | 12.4 |
| BE-05 | LIKE pattern not escaped | 12.5 |
| BE-06 | Race condition outbox | 12.6 |
| BE-07 | SSE broadcaster memory leak | 12.7 |
| BE-08 | Missing job locking | 12.8 |
| BE-09 | Stale embedding no max retries | 16.2 |
| BE-10 | Cache invalidation gap | 16.3 |
| BE-11 | N+1 over-fetching in search | 16.4 |
| BE-12 | KAG routes leak existence | 16.5 |
| BE-13 | DB pool missing error handler | 12.9 |
| BE-14 | Worker no shutdown timeout | 12.10 |
| BE-15 | Ingestion pipeline | 11.2, 11.3 |
| BE-16 | ACL filtering | 11.7 |
| BE-17 | Hybrid search | 11.6 |
| FE-01 | No error boundaries | 13.1 |
| FE-02 | No server-side auth | 13.2 |
| FE-03 | Missing security headers | 13.3 |
| FE-04 | CSRF token race | 13.4 |
| FE-05 | EventSource memory leak | 13.5 |
| FE-06 | No unified data fetching | 13.6 |
| FE-07 | Refresh mechanisms conflict | 13.7 |
| FE-08 | Missing Suspense | 13.8 |
| FE-09 | Missing page metadata | 13.9 |
| FE-10 | useMobile undefined | 14.9 |
| FE-11 | No retry/backoff | 16.6 |
| DS-01 | Dual animation systems | 14.1 |
| DS-02 | Sheet animation > budget | 14.2 |
| DS-03 | Sidebar bypasses MOTION | 14.2 |
| DS-04 | No prefers-reduced-motion | 14.3 |
| DS-05 | Skeleton infinite loop | 16.7 |
| DS-06 | No FormField component | 14.4 |
| DS-07 | Toast no stacking | 14.5 |
| DS-08 | Missing Dialog/Modal | 14.6 |
| DS-09 | No Pagination | 14.7 |
| DS-10 | Toast no aria-live | 14.5 |
| DS-11 | Z-index not formalized | 14.10 |
| DS-12 | Form labels not associated | 14.4 |
| DS-13 | Checkbox not Radix | 14.8 |

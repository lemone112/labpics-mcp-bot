...
function accountId(env) {
  return String(env.CHATWOOT_ACCOUNT_ID);
}

async function resolveProjectId(env, conversationGlobalId) {
  const rows = await sb(env, 'GET', `/rest/v1/project_conversation_map?conversation_global_id=eq.${encodeURIComponent(conversationGlobalId)}&select=project_id&limit=1`);
  return rows && rows[0] && rows[0].project_id ? String(rows[0].project_id) : null;
}
...
async function getWatermark(env, projectId) {
  const table = env.SYNC_TABLE || 'rag_chatwoot_sync_state';
  const qs = `?project_id=eq.${encodeURIComponent(projectId)}` +
    `&source=eq.chatwoot` +
    `&account_id=eq.${encodeURIComponent(accountId(env))}` +
    `&select=project_id,source,account_id,last_processed_at,last_processed_message_id,status,error,meta,last_run_at,last_success_at` +
    `&limit=1`;
...
async function upsertWatermark(env, projectId, patch) {
  const table = env.SYNC_TABLE || 'rag_chatwoot_sync_state';
  const row = Object.assign({ project_id: projectId, source: 'chatwoot', account_id: accountId(env), status: 'ok', error: null, meta: {} }, patch || {});
  await sb(env, 'POST', `/rest/v1/${table}?on_conflict=project_id,source,account_id`, row, { prefer: 'resolution=merge-duplicates,return=minimal' });
}


async function upsertConversation(env, projectId, convo) {
  const row = {
    project_id: projectId,
    conversation_global_id: convG(accountId(env), convo.id),
...
async function upsertMessage(env, projectId, conversationGlobalId, msg) {
  const row = {
    project_id: projectId,
    conversation_global_id: conversationGlobalId,
    message_global_id: msgG(accountId(env), msg.id),
...
async function createPendingChunks(env, projectId, conversationGlobalId, messageGlobalId, content, sourceUpdatedAtISO) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    const material = `${projectId}|chatwoot|${messageGlobalId}|${i}|${model}|${chunks[i]}`;
...
async function runSync(env) {
  ...
  const convosResp = await cw(env, `/api/v1/accounts/${accountId(env)}/conversations?sort_by=last_activity_at&order_by=desc&page=1`);
  ...
  for (const convo of picked) {
    const conversationGlobalId = convG(accountId(env), convo.id);
    const projectId = await resolveProjectId(env, conversationGlobalId);
    if (!projectId) { skippedConvos++; continue; }

    const wm = await getWatermark(env, projectId);
    ...
    await upsertWatermark(env, projectId, { status: 'running', error: null, last_run_at: new Date().toISOString(), meta: Object.assign({}, (wm && wm.meta) || {}, { mode: 'cron' }) });

    await upsertConversation(env, projectId, convo);
    processedConvos++;

    const msgsResp = await cw(env, `/api/v1/accounts/${accountId(env)}/conversations/${convo.id}/messages`);
    ...
    for (const msg of tail) {
      ...
      const messageGlobalId = await upsertMessage(env, projectId, conversationGlobalId, msg);
      processedMsgs++;

      if (!msg.private) {
        const text = String(msg.content || '').trim();
        if (text.length >= minLen) {
          pendingChunks += await createPendingChunks(env, projectId, conversationGlobalId, messageGlobalId, text, createdISO);
        }
      }

      if (!newestSeen || (createdISO && createdISO > newestSeen)) { newestSeen = createdISO; newestMsgGlobalId = messageGlobalId; }
    }

    await upsertWatermark(env, projectId, {
      status: 'ok', error: null, last_processed_at: newestSeen, last_processed_message_id: newestMsgGlobalId, last_success_at: new Date().toISOString(),
      meta: Object.assign({}, (wm && wm.meta) || {}, { run_finished_at: new Date().toISOString(), processed_conversations: 1, processed_messages: processedMsgs, pending_chunks_created: pendingChunks, skipped_conversations: skippedConvos })
    });
  }

  return { ok: true, processedConvos, processedMsgs, pendingChunks, skippedConvos };
}
